...
With [PyTorch Distributions](https://pytorch.org/docs/stable/distributions.html), implementing these distributions is easy! We model the encoder using the following encoding layers:
...
and then, using the `Categorical` distribution inside torch:
```python
