...
import SampleAndProjectPlot from './samplingAndProjecting' -->
...
## A naïve approach to defining a normal distribution on the circle
...
<!-- Sampling and projecting -->
[Sampling and projecting]

Notice, however, that this approach is somewhat unsatisfactory: we relied on having an ambient space to start with (i.e. having our circle $\mathbb{S}^1$ **embedded** in $\mathbb{R}^2$). This is *no bueno* in differential geometry, since we usually like to deal with manifolds as intrinsic objects, not as objects living inside some ambient space.[^Nash-is-cool]
...
[ Circle and Line with Gaussian on top ]
...
[maybe shooting vectors]

This intuitive process of walking in a certain direction is properly formalized in the context of Riemannian manifolds using **geodesics** and the **exponential map**.
...
[ image of arc path with geodesic ]

This is your first example of **the exponential map**. More formally, there is a mapping between pairs of points and tangent vectors $(p, \bm{v}_p) \in \mathcal{M}\times T_p\mathcal{M}$ and points $q$ in the manifold. In the case of Riemannian Manifolds, we have a unique geodesic (i.e. distance minimizing curve) $\gamma(t)$ that satisfies $\gamma(0) = p$ and that starts with velocity $\dot{\gamma}(0) = \bm{v}_p$. As we just discussed, the way this curve looks in the circle is just $(\cos(\lambda t + t_0), \sin(\lambda t + t_0))$ where $t_0$ is chosen so that we "start" at $p$, and $\lambda$ is the coefficient of $\bm{v}_p$.[^the-details]
...
$$
that takes tangent vectors to the manifold and maps them into points. The point you land on when following the tangent vector $\bm{v}_\bm{\mu}$ for a unit of time is denoted as $\exp_\bm{\mu}(v_\bm{\mu})$.
...
The only difference with the treatment we have given it so far is going from a definition on $\mathbb{R}$ to higher dimensional spaces.

## More examples: sphere and torus

The good news are: some people have already computed the exponential map for many different Riemannian manifolds. In the case of the sphere $\mathbb{S}^2$, the tangent space is actually a tangent plane, and the exponential at a given point $\bm{\mu}\in\mathbb{S}^2$ on the direction of a tangent vector $\bm{v}_\bm{\mu}$ is given by

[ equation ]

so we can consider the wrapped Gaussian distribution of e.g. a unit Gaussian $N_{\mathbb{S}^{2}}(0, I_2)$ (where $I_2$ is the identity matrix).
