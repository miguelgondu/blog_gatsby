...
This blogpost introduces Bayesian Optimization, and provides code snippets for the experiments performed. The tools used include GPyTorch and BoTorch as the main engines for Gaussian Processes and BO, and EvoTorch for the evolutionary strategies.
...
<p align="center">
  <img src={"/assets/bo_blogpost/all_three_test_functions.jpg"}  width="100%"/>
</p>
...
Assuming $f\sim \text{GP}(\mu, k)$ allows for computing **an entire distribution** over newly seen points. As an example, let's say you have a dataset $\mathcal{D} = \{(x_1, f(x_1)), \dots, (x_N, f(x_N))\}$ for our test function $f(x)=x\sin(x)$, measured with a little bit of noise: 

<p align="center">
  <img src={"/assets/bo_blogpost/xsinx_w_noisy_samples.jpg"}  width="100%"/>
</p>

Given a new point $x_*$, you can consider the joint distribution of $\{f(x_1), \dots, f(x_N), f(x_*)\}$ and compute the conditional distribution of $f(x_*)$ given the dataset:[^the-ugly-details]
...
Let's visualize this distribution for all points in the interval [-10, 10], highlighting what happens at e.g. $x=7$:

[TODO: finish this plot]

<p align="center">
  <img src={"/assets/bo_blogpost/xsinx_gp_fit.jpg"}  width="100%"/>
</p>
...
Gaussian Processes, as well as other nonparametric methods for regression, allow us to sample from "the space of functions". We can sample a given instance of our approximation:

[TODO: plots with samples]

This space of functions is determined by our choice of kernel, which spans a "Reproducing Kernel Hilbert Space" (RKHS).[^ugly-detail-RKHS] Common choices of kernels are the RBF and the Matérn family:

[^ugly-detail-RKHS]: Hilbert Spaces are a special kind of vector space, which have both a dot product and a topology that is complete w.r.t. this dot product. If you are curious about the formalities, check [TODO: add reference]. 
...
## Gaussian Processes in practice: Scikit-learn and GPyTorch

[A section about sklearn, GPyTorch]

[Other tools]

As discussed in the introduction, this presentation of GPs is quick and to-the-point. Slower and more thorough introductions can be found in the references. [TODO: add the two blogposts, and the relevant chapters in GPML].
...
As we discussed in the introduction, examples of black-box objective functions include [TODO: add]. Now we discuss four examples of acquisition functions.
...
Circling back to our guiding example of $x\sin(x)$, this would be the candidate proposed by Thompson Sampling (TS):

[Plot with one sample]

### Improvement-based policies

Since we have estimates of the posterior mean $\mu$ and the posterior variance $\sigma^2$ at each step, we have access to a lot more than just a single sample (as in TS). For each new point $\bm{x}$, consider its "improvement":

$$ I(\bm{x}; \mathcal{D}) = \max(0, f(\bm{x}) - f_{\text{best}})$$

where $f_{\text{best}}$ is the maximum value in our trace $\mathcal{D} = \{(\bm{x}_n, f(\bm{x}_n))\}$. The improvement measures how much we are gaining at point $f(\bm{x})$, compared to the current best.
...
Given the running example, here are the improvement-based policies visually. [TODO: add and explain].
